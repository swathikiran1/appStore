...
class AppStore extends Component {
  state = {searchInput: '', activeTabCategory: appsList[0].category}
  ...
  render() {
    const {searchInput, activeTabCategory} = this.state
    const filteredAppList = appsList.filter(
      eachItem =>
        eachItem.category === activeTabCategory &&
      ...
      <div className="app-container">
        ...
        <ul className="tabs-list">
          {tabsList.map(eachItem => (
            <TabItem
              tabItem={eachItem}
              onClickTab={this.changeTabItem}
              key={eachItem.tabId}
              isActive={activeTabCategory === eachItem.tabId}
            />
          ))}
        </ul>
